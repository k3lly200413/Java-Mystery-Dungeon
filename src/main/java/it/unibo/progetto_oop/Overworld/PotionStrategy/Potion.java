package it.unibo.progetto_oop.Overworld.PotionStrategy;

import it.unibo.progetto_oop.Overworld.AdapterPattern.PossibleUser;
import it.unibo.progetto_oop.Combat.Inventory.Item;
import it.unibo.progetto_oop.Combat.Position.Position;

public class Potion extends Item{

    private final PotionEffectStrategy strategy;

    /**
     * 
     * @param name Nome che descrive la pozione
     * @param description Descrizione di funzione della pozione
     * @param quantity Quantità nell'inventario
     * @param strategy Istanza di Pozione necessaria per chiamare i metodi necessari
     */
    Potion( String name, String description, Position position, PotionEffectStrategy strategy) {
        super(name, description, position);
        this.strategy = strategy;
    }

    /**
     * @return the strategy
     */
    public PotionEffectStrategy getStrategy(){
        return this.strategy;
    }

    /**
     * 
     * @param target Istance of CombatModel to call necessary functions to change player details
     */
    @Override
    public boolean use(PossibleUser target) {
        if (this.strategy != null) {
            System.out.println("Applying effect of: " + this.getName());
            this.strategy.apllyEffect(target);
            return true;
        } else {
            System.out.println("Warning: Potion " + getName() + " has no strategy.");
            return false;
        }
    }
}
